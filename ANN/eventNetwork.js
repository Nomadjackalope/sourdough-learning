...
function sigmoid(val) {
    ...
    var retVal = 1 / (1 + Math.pow(2.718, -val));
...
function dsigmoid(val) {
    ...
    if(val < -1 || retVal > 1) {
        var ethreex = Math.pow(2.718, 3 * val);
    ...
    // if(curNeuron.activatedPct <= neuronPctLimit) {
        ...
        if(curNeuron.activatedVal >= activationLimit) {
            ...
            while(curNeuron.numConnectionsOutUsed < curNeuron.connectionsOut.length) {
                ...
                activate(curNeuron.connectionsOut[curNeuron.numConnectionsOutUsed], curNeuron.activatedVal);
...
function trainNodes(input, target) {
		...
		var error = [target[0] - neurons[7].activatedVal, target[1] - neurons[8].activatedVal];
...
function test(input, expectedVal) {
	...
	if(Date.now() - startTime >= runtime) {
		...
		// Run forward prop
		beginningNeuronActivation([neurons[0], neurons[1], neurons[2]], input);
		console.log(input + ": ", neurons[7].activatedVal, neurons[8].activatedVal, " <- should = " + expectedVal);
